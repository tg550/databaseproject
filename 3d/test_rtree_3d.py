...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 24 01:13:35 2018
...
class RectangleGen(object):
    ...
    def rect(self, size=10.0):
        x,y,z,w,h,l = rr(),rr(),rr(),random.uniform(0.0,size),random.uniform(0.0,size),random.uniform(0.0,size)
        r = Rect(x,y,z,x+w,y+h,z+l)
        assert(not r.swapped_x)
        assert(not r.swapped_y)
        assert(not r.swapped_z)
        return r

    def intersectingWith(self,ra):
        rb_x = random.uniform(ra.x,ra.xx)
        rb_y = random.uniform(ra.y,ra.yy)
        rb_z = random.uniform(ra.z,ra.zz)
        return Rect(rb_x,rb_y,rb_z,rb_x + rr(),rb_y + rr(),rb_z + rr())

    def disjointWith(self, ra):
        ax,ay,az,aw,ah,al = ra.extent()
        w,h,l = rr(),rr(),rr()
        distsq = max(w*w + h*h + l*l, aw*aw + ah*ah + al*al)
        dist = 2.0 * math.sqrt(distsq) + random.uniform(0.1, 1.0)
        ang = random.uniform(0.0, 2.0 * math.pi)
        x = math.cos(ang) * dist
        y = math.sin(ang) * dist
        z = math.sin(ang) * dist
        return Rect(ax+x,ay+y,az+z,ax+x+w,ay+y+h,az+z+l)

    def pointInside(self, r):
        return (random.uniform(r.x,r.xx),random.uniform(r.y,r.yy),random.uniform(r.z,r.zz))
...
class RectangleTests(ut.TestCase):
    def testCons(self):
        r = Rect(0,0,0,10,10,10)
        self.assertTrue(r is not None)
        self.assertTrue(r is not NullRect)

    
    def testIntersection(self):
        ra = Rect(0,0,0,10,10,10)
        rb = Rect(5,5,5,15,15,15)
        res = ra.intersect(rb)
        x,y,z,w,h,l = res.extent()
        ...
        self.assertEqual(res.area(), 125)

        rc = Rect(0,0,0,10,10,10)
        rd = Rect(11,11,11,21,21,21)
    ...
    def testUnion(self):
        ra = Rect(0,0,0,10,10,10)
        rb = Rect(-10,-10,-10,1,1,1)
        x,y,z,w,h,l = ra.union(rb).extent()
...
class RTreeTest(ut.TestCase):
    ...
    def _invariants(self,node, seen):
        ...
        r = Rect(node.rect.x, node.rect.y, node.rect.z, node.rect.xx, node.rect.yy,node.rect.zz)
        for c in node.children():
    ...
    def testContainer(self):
        ...
        xs = [ TstO(r) for r in take(10,G.rect, 0.1) ]
        tree = RTree()
        for x in xs: 
    ...
    def testRectQuery(self):
        xs = [ TstO(r) for r in take(100, G.rect, 0.01) ]
        rt = RTree()
        for x in xs: 
            rt.insert(x,x.rect)
            self.invariants(rt)

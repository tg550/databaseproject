...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr 22 22:30:56 2018
...
from rect_3d import Rect, union_all, NullRect
...
class RTree(object):
    ...
    def _ensure_pool(self, idx):
        if len(self.rect_pool) < (6*idx):
            self.rect_pool.extend([0,0,0,0,0,0] * idx)
            self.node_pool.extend([0,0,0] * idx)
...
class _NodeCursor(object):
    ...
    def create_with_children(cls, children, rooto):
        ...
        nr = Rect(rect.x,rect.y,rect.z,rect.xx,rect.yy,rect.zz)
    ...
    def create_leaf(cls, rooto, leaf_obj, leaf_rect):
        rect = Rect(leaf_rect.x,leaf_rect.y,leaf_rect.z,leaf_rect.xx,leaf_rect.yy,leaf_rect.zz)
    ...
    def _become(self, index):
        ...
        if (x == 0.0 and y == 0.0 and z == 0.0 and xx == 0.0 and yy == 0.0 and zz == 0.0): 
            self.rect = NullRect
        else:
            self.rect = Rect(x,y,z,xx,yy,zz)
    ...
    def insert(self, leafo, leafrect):
        ...
        while True:
            if self.holds_leaves():
                ...
                minarea = -1.0
                for c in self.children():
                    x,y,z,xx,yy,zz = c.rect.coords()
                    lx,ly,lz,lxx,lyy,lzz = leafrect.coords()
                    nx = x if x < lx else lx
                    nxx = xx if xx > lxx else lxx
                    ny = y if y < ly else ly
                    nyy = yy if yy > lyy else lyy
                    nz = z if z < lz else lz
                    nzz = yy if zz > lzz else lzz                    
                    a = (nxx - nx) * (nyy - ny) * (nzz - nz)
...
def closest(centroids, node):
    x,y,z = center_of_gravity([node])
    dist = -1
    ridx = -1

    for (i,(xx,yy,zz)) in enumerate(centroids):
        dsq = ((xx-x) ** 2) + ((yy-y) ** 2 ) + ((zz-z) ** 2)
